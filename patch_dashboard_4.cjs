const fs = require('fs');
let content = fs.readFileSync('src/hooks/useDashboardData.js', 'utf8');

const codeToInject = `  const itikPetelurOverviewData = useMemo(() => {
...
content = content.replace(/  return \{[\s\S]*?\}\s*\}\s*$/, '  return {\n    currentYear,\n    isSalesProgram,\n    maggotBioconversionData,\n    maggotFinancialData,\n    maggotPortfolioData,\n    maggotYTD,\n    dynamicSalesChartData,\n    totalWasteManaged,\n    totalOmzet,\n    fisheryOverviewData,\n    fisheryPortfolioRaw,\n    fisheryYTD,\n    pembibitanOverviewData,\n    pembibitanYTD,\n    quailOverviewData,\n    quailYTD,\n    tempeOverviewData,\n    tempeYTD,\n    ecogrowOverviewData,\n    ecogrowYTD,\n    cahayaTaniOverviewData,\n    cahayaTaniYTD,\n    itikPetelurOverviewData,\n    itikPetelurYTD\n  };\n}');

// Let's make absolutely sure we don't duplicate it.
if (!content.includes('const itikPetelurOverviewData = useMemo(() => {')) {
  const returnIndex = content.lastIndexOf('  return {');
  if (returnIndex !== -1) {
    content = content.slice(0, returnIndex) + codeToInject + '\n\n' + content.slice(returnIndex);
  }
}

fs.writeFileSync('src/hooks/useDashboardData.js', content);
console.log('Patched');
