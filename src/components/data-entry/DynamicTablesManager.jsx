...
export const LingkarTambangPanenTable = ({ title, value = [], onChange }) => {
  const headers = ["Item / Produk", "Qty (Buah)", "Qty (Kg)"];
  const initialLabels = ["Lainnya"];
          ...
          <tbody>
            {value.map((row) => (
              <tr key={row.id} className="border-b border-gray-100 hover:bg-blue-50/60">
                ...
                <td className="px-3 py-2.5">
                  <input type="number" className="w-full border border-gray-300 p-1.5 rounded focus:border-[#25326a]" value={row.qty_buah} onChange={(e) => handleFieldChange(row.id, 'qty_buah', e.target.value)} />
                </td>
                <td className="px-3 py-2.5">
                  <input type="number" className="w-full border border-gray-300 p-1.5 rounded focus:border-[#25326a]" value={row.qty_kg} onChange={(e) => handleFieldChange(row.id, 'qty_kg', e.target.value)} />
                </td>
              </tr>
            ))}
          </tbody>
...
export const LingkarTambangPenjualanTable = ({ title, value = [], onChange }) => {
  const headers = ["Item / Produk", "Qty (Buah)", "Qty (Kg)", "Harga Satuan (Rp/Kg)", "Omzet"];
  const initialLabels = ["Lainnya"];
  ...
  const handleFieldChange = (id, field, val) => {
    ...
    if (field === 'qty_kg' || field === 'unit_price') {
      const q = parseFloat(field === 'qty_kg' ? val : updatedRow.qty_kg);
      const p = parseFloat(field === 'unit_price' ? val : updatedRow.unit_price);
      if (!isNaN(q) && !isNaN(p)) {
        updatedRow.revenue = (q * p).toString();
      } else {
...
export default function DynamicTablesManager({ selectedProgram, tablesData, handleTableChange, extraFields, handleExtraFieldChange }) {
  ...
  if (p.includes("puyuh")) {
    return (
      <div className="flex flex-col gap-6 w-full">
        <div className="flex gap-4">
          <div className="flex-1">
            <label className="block text-sm font-bold text-gray-800 mb-1.5">Populasi Hewan Ternak (Ekor)</label>
            <input type="number" className="w-full border border-gray-300 p-2.5 rounded-sm focus:outline-none focus:border-[#25326a]" value={extraFields['Populasi Hewan Ternak'] || ''} onChange={e => handleExtraFieldChange('Populasi Hewan Ternak', e.target.value)} />
          </div>
        </div>
        <GenericDynamicTable 
  ...
  if (p.includes("sirah pulau")) {
    return (
      <div className="flex flex-col gap-6 w-full">
        ...
        <GenericDynamicTable 
          title="Penjualan Telur Ayam" 
          headers={["Produk", "Qty (Butir)", "Harga Satuan (Rp)", "Omzet"]} 
          initialLabels={["Telur Mentah", "Lainnya"]} 
          value={tablesData['telur']}
          onChange={(rows) => handleTableChange('telur', rows)}
        />
        <GenericDynamicTable 
  ...
  if (p.includes("lingkar tambang")) {
    return (
      <div className="flex flex-col gap-4 w-full">
        <LingkarTambangPanenTable 
          title="Produksi / Hasil Panen" 
          value={tablesData['panen']}
          onChange={(rows) => handleTableChange('panen', rows)}
        />
        <LingkarTambangPenjualanTable 
          title="Penjualan" 
