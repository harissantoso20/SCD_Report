...
export default function AdvancedFishAnalytics() {
  ...
  const generateCrossRevenueAI = () => {
    ...
    return (
      <div className="text-gray-600 font-medium leading-relaxed text-[13px] space-y-4">
        <p>
          Secara bulanan, puncak gairah pasar terjadi pada bulan <span className="font-bold text-[#1e3a8a]">{maxBulan}</span> dengan total omzet tertinggi menembus <span className="font-bold text-emerald-600">{formatJuta(maxOmzet)}</span>.
        </p>
        <p>
          Secara keseluruhan dalam periode <span className="font-bold text-[#1e3a8a]">{filteredOverviewData[0].month} - {filteredOverviewData[filteredOverviewData.length - 1].month} {currentYear}</span>, total pendapatan mencapai <span className="font-bold text-emerald-600">{formatRupiah(total)}</span>. Portofolio bisnis saat ini sangat condong pada penjualan <span className="font-bold text-blue-600">{winner}</span>, yang menguasai <span className="font-bold text-slate-800">{winnerPct}%</span> dari kue pendapatan.
        </p>
        <p>
        ...
        <div className="mt-4 bg-blue-50/50 p-3 rounded border border-blue-100 flex gap-3 items-start">
  ...
  const renderLabelKg = (props) => {
    ...
    if (!value) return null;
    return (
      <text x={x + width / 2} y={y - 8} fill="#6366f1" fontSize={10} fontWeight="bold" textAnchor="middle">
    ...
    <div className="w-full flex flex-col gap-6 font-sans text-slate-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 border-t-4 border-t-indigo-500">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Total Ikan Konsumsi YTD</p>
          <h2 className="text-3xl font-extrabold text-slate-800">
            {new Intl.NumberFormat('id-ID').format(fisheryYTD.total_konsumsi_kg)} <span className="text-lg text-slate-500 font-semibold">Kg</span>
          </h2>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 border-t-4 border-t-amber-500">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Total Bibit Ikan YTD</p>
          <h2 className="text-3xl font-extrabold text-slate-800">
            {new Intl.NumberFormat('id-ID').format(fisheryYTD.total_bibit_ekor)} <span className="text-lg text-slate-500 font-semibold">Ekor</span>
          </h2>
        </div>
      </div>
                ...
                <div className="h-48 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={filteredOverviewData} margin={{ top: 25, right: 10, left: 10, bottom: 0 }}>
                      ...
                      <Bar dataKey="konsumsi_kg" name="Ikan Konsumsi" fill="#6366f1" radius={[4, 4, 0, 0]} barSize={24} label={renderLabelKg} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
                ...
                <div className="h-48 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={filteredOverviewData} margin={{ top: 25, right: 10, left: 10, bottom: 0 }}>
                      ...
                      <YAxis hide={true} domain={[0, 'dataMax * 1.2']} />
                      <Tooltip cursor={{ fill: '#f8fafc' }} contentStyle={{ borderRadius: '8px', border: '1px solid #e2e8f0', fontSize: '12px' }} />
                      <Bar dataKey="bibit_ekor" name="Bibit Ikan" fill="#f59e0b" radius={[4, 4, 0, 0]} barSize={24} label={renderLabelEkor} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <table className="w-full text-sm border-collapse border border-slate-100 rounded-lg overflow-hidden">
                <thead>
                  <tr className="bg-slate-50">
                    ...
                    <th className="p-4 text-center font-bold text-[#1e3a8a] uppercase tracking-wider border-b border-slate-200">Persentase</th>
                  </tr>
                </thead>
                <tbody>
                  {YTD_TABLE_DATA.map((row, idx) => (
                    <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                      ...
                      <td className={`p-4 text-center font-bold ${row.percent.startsWith('+') ? 'text-emerald-600' : 'text-red-500'}`}>
                        {row.percent}
                      </td>
                    </tr>
                  ))}
