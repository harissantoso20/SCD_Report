...
export default function AdvancedPembibitanAnalytics() {
  ...
  const renderGaugeChart = () => {
    const data = [
      { name: 'Terjual', value: Number(stockToSalesRatio), fill: '#f59e0b' },
    ...
    return (
      <div className="relative w-full h-[180px] flex items-center justify-center">
        ...
        <div className="absolute bottom-4 flex flex-col items-center">
          <span className={`text-2xl font-extrabold ${Number(stockToSalesRatio) < 20 ? 'text-red-500' : 'text-slate-700'}`}>{stockToSalesRatio}%</span>
  ...
  const renderAIInsight = () => {
    ...
    let omzetText = '';
    if (deltaOmzet > 0) {
      omzetText = <span className="font-medium text-emerald-600">Dibandingkan dengan tahun sebelumnya ({prevYear}), performa tahun ini mencatat lonjakan omzet sebesar {formatRupiah(Math.abs(deltaOmzet))}, mengonfirmasi tren pertumbuhan komersial yang sangat positif.</span>;
    } else if (deltaOmzet < 0) {
      omzetText = <span>Namun jika dibandingkan dengan tahun sebelumnya ({prevYear}), performa komersial mengalami kontraksi parah. Terdapat <span className="font-bold text-red-500">penurunan omzet (Lost Revenue) sebesar {formatRupiah(Math.abs(deltaOmzet))}</span>, yang mensyaratkan manajemen untuk segera turun tangan membenahi rantai pasok dan mencari *offtaker* baru.</span>;
    } else {
    ...
    if (peakMonthData && peakMonthData.total_rp > 0) {
      peakStatement = <p>Secara bulanan, omzet tertinggi pada periode ini berhasil dicetak pada bulan <span className="font-bold text-[#1e3a8a]">{peakMonthData.month}</span> senilai <span className="font-bold text-emerald-600">{formatRupiah(peakMonthData.total_rp)}</span>.</p>;
    }
    ...
    <div className="w-full flex flex-col gap-6 font-sans text-slate-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 border-t-4 border-t-emerald-500">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Total Revenue YTD</p>
          <h2 className="text-3xl font-extrabold text-slate-800">
            {formatRupiah(pembibitanYTD.total_omzet)}
          </h2>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 border-t-4 border-t-fuchsia-500">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Total Penjualan Bibit YTD</p>
          <h2 className="text-3xl font-extrabold text-slate-800">
            {formatNumber(pembibitanYTD.total_penjualan)} <span className="text-lg text-slate-500 font-semibold">Batang</span>
          </h2>
        </div>
        <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 border-t-4 border-t-blue-500">
          <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">Stok Pembesaran Aktif</p>
          <h2 className="text-3xl font-extrabold text-slate-800">
            {formatNumber(pembibitanYTD.total_pembesaran)} <span className="text-lg text-slate-500 font-semibold">Batang</span>
          </h2>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-stretch">
        ...
        <section className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 xl:col-span-1 flex flex-col h-full relative overflow-hidden group hover:-translate-y-1 hover:shadow-md transition-all duration-300">
        ...
        <div className="xl:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <section className="bg-white rounded-md shadow-sm border border-slate-200 p-5 md:p-6 md:col-span-2">
            <h3 className="text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-4">Overview Produksi</h3>
            <div className="h-[250px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={filteredOverviewData} margin={{ top: 20, right: 10, left: 0, bottom: 5 }}>
                  ...
                  <Bar yAxisId="left" dataKey="penjualan_batang" name="Penjualan (Batang)" fill="#f59e0b" radius={[4, 4, 0, 0]} maxBarSize={40}>
                    <LabelList dataKey="penjualan_batang" position="top" style={{fontSize: '10px', fill: '#d97706', fontWeight: 'bold'}} formatter={(v) => v > 0 ? formatNumber(v) : ''} />
                  </Bar>
                  <Line yAxisId="right" type="monotone" dataKey="total_rp" name="Omzet (Rp)" stroke="#10b981" strokeWidth={3} dot={{ r: 4, fill: '#10b981', strokeWidth: 2, stroke: '#fff' }}>
                    <LabelList dataKey="total_rp" position="top" style={{fontSize: '10px', fill: '#059669', fontWeight: 'bold'}} formatter={(v) => v > 0 ? formatJuta(v) : ''} />
                  </Line>
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          ...
          <section className="bg-white rounded-md shadow-sm border border-slate-200 p-5">
            <h3 className="text-[12px] font-bold text-slate-500 uppercase tracking-widest text-center mb-2">Stock to Sales Ratio</h3>
            {renderGaugeChart()}
          </section>

          <section className="bg-white rounded-md shadow-sm border border-slate-200 p-5 flex flex-col justify-between">
            <h3 className="text-[12px] font-bold text-slate-500 uppercase tracking-widest text-center mb-4">Pricing Trend (Harga Bibit Rata-Rata)</h3>
            <div className="h-[120px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={filteredOverviewData} margin={{ top: 10, right: 10, left: 10, bottom: 5 }}>
                  ...
                  <Line type="monotone" dataKey="avg_harga_bibit" stroke="#3b82f6" strokeWidth={3} dot={{ r: 4, fill: '#3b82f6', strokeWidth: 2, stroke: '#fff' }}>
                    <LabelList dataKey="avg_harga_bibit" position="top" style={{fontSize: '10px', fill: '#3b82f6', fontWeight: 'bold'}} formatter={(v) => v > 0 ? formatRupiah(v) : ''} />
                  </Line>
                </LineChart>
              </ResponsiveContainer>
            </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <section className="bg-white rounded-md shadow-sm border border-slate-200 p-5 xl:col-span-1">
            <h3 className="text-[12px] font-bold text-slate-500 uppercase tracking-widest text-center mb-4">Ancillary Revenue</h3>
            <div className="h-[200px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={[
                      { name: 'Omzet Bibit', value: pembibitanYTD.total_bibit_omzet, fill: '#f59e0b' },
                      { name: 'Omzet Polybag', value: pembibitanYTD.total_polybag_omzet, fill: '#10b981' }
                    ].filter(d => d.value > 0)}
                    cx="50%"
                    cy="50%"
                    innerRadius={40}
        ...
        <section className="bg-white rounded-md shadow-sm border border-slate-200 p-5 md:p-6 xl:col-span-2">
          <h2 className="text-[14px] font-bold text-[#25326a] uppercase tracking-wider border-b border-gray-200 pb-2 mb-5">
            ...
            <table className="w-full text-sm text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  <th className="p-4 text-left font-bold text-[#1e3a8a] uppercase tracking-wider">Variabel</th>
                  <th className="p-4 text-center font-bold text-[#1e3a8a] uppercase tracking-wider">{prevYear}</th>
                  <th className="p-4 text-center font-bold text-[#1e3a8a] uppercase tracking-wider">{currentYear}</th>
                  <th className="p-4 text-center font-bold text-[#1e3a8a] uppercase tracking-wider">Persentase</th>
                </tr>
              </thead>
              <tbody>
                {YTD_TABLE_DATA.map((row, idx) => (
                  <tr key={idx} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4 text-center text-slate-500 font-medium">{row.prevYearData.toLocaleString('id-ID')}</td>
                    <td className="p-4 text-center font-extrabold text-slate-800">{row.currYearData.toLocaleString('id-ID')}</td>
                    <td className={`p-4 text-center font-bold ${row.percent.startsWith('+') ? 'text-emerald-600' : (row.percent === '0%' ? 'text-slate-400' : 'text-red-500')}`}>
                      {row.percent}
                    </td>
                  </tr>
                ))}
