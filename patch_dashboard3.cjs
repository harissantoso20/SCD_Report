const fs = require('fs');
let content = fs.readFileSync('src/hooks/useDashboardData.js', 'utf-8');
...
let searchRegex = /  return \{\r?\n    currentYear,/g;
content = content.replace(searchRegex, insertStr);

const replaceExports = `    ecogrowYTD,
    cahayaTaniOverviewData,
    cahayaTaniYTD
  };`;
content = content.replace(/    ecogrowYTD\r?\n  \};/g, replaceExports);
